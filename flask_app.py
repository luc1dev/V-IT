...
from bot_api.connection import confirmation_token, confirmation_token_ingut
from bot_api.vk_bot_response import *
from db_utils.database import Menu, Inheritances
...
@app.route('/ingut', methods=['POST'])
def processing_ingut():
    try:
        # Распаковываем json из пришедшего POST-запроса
        data = json.loads(request.data)
        # Вконтакте в своих запросах всегда отправляет поле типа
        if 'type' not in data.keys():
            return 'not vk'
        if data['type'] == 'confirmation':
            return confirmation_token_ingut
        elif data['type'] == 'message_new':
            text = str(data['object']['message']['text'])
            peer_id = str(data['object']['message']['peer_id'])
            bot_response_ingut(peer_id=peer_id, user_request=text)
            return 'ok'
    except Exception as e:
        print(f'{e}')
